...
import type { DivisionType } from '@/types/division/divisionTypes'
...
type DivisionTypeWithAction = DivisionType & {
  action?: string
}
...
// Column Definitions
const columnHelper = createColumnHelper<DivisionTypeWithAction>()
...
const DivisionTable = () => {
  ...
  const [divisionData, setDivisionData] = useState<DivisionType[]>([])
  ...
  const handleOpenDeleteConfirm = (departmentId: string) => {
    setSelectedDivisionId(departmentId)
  ...
  const confirmDeleteDepartment = async () => {
    if (selectedDivisionId) {
      try {
        const response = await axios.delete(`/department/${selectedDivisionId}`)
  ...
  const columns = useMemo<ColumnDef<DivisionTypeWithAction, any>[]>(
    () => [
      {
        id: 'select',
        header: ({ table }) => (
              ...
              options={[
                {
                  ...
                  menuItemProps: {
                    ...
                    onClick: () => router.push(`/department/${row.original.divisionId}`)
                  }
                },
                {
                ...
                },
                {
                  ...
                  menuItemProps: {
                    className: 'flex items-center gap-2 text-textSecondary',
                    onClick: () => handleOpenDeleteConfirm(row.original.divisionId)
                  }
                }
              ]}
            />
        ...
        <div className='flex justify-between flex-col items-start md:flex-row md:items-center p-6 border-bs gap-4'>
          ...
          <div className='flex flex-col sm:flex-row is-full sm:is-auto items-start sm:items-center gap-4'>
            ...
            <Button
              ...
              className='is-full sm:is-auto'
              onClick={() => router.push('/division/new')}
            >
              Add New Division
            </Button>
          </div>
